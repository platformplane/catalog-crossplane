...
RUN if [ -z "$IMAGE_NAME" ]; then echo 'IMAGE_NAME is not set, docker build must be called with --build-arg IMAGE_NAME=$YOUR_IMAGE_NAME' && exit 1; else echo "IMAGE_NAME is set to $IMAGE_NAME"; fi
...
RUN crossplane xpkg push -f catalog-items.xpkg $REGISTRY_IMAGE/$IMAGE_NAME:$IMAGE_VERSION
